...
window.VS_DATA = {
  ...
  sections: [
    ...
    },
    {
      ...
      sub: [
        ...
        { title: "2.6.2  Live attribution during capture", intro: "Audio is captured from the event's English stream and transcribed live with per-word speaker diarization. Each transcript segment is split into runs of a single diarizer speaker index, so no stored utterance mixes two voices. Every utterance is stored with its diarizer index, its position in the stream, and its timestamp.", items: [
          ["Voice confirmation", "an index is confirmed as a roster speaker only by voice: after at least 30 seconds of that index's audio, several short windows are embedded, averaged and compared to the roster prints. A cosine distance below 0.55 confirms the index; anything at or above 0.55 confirms nothing. A confirmation at 0.40 or above is re-scored on twelve samples spread across everything the index has said and kept only if it is still below 0.55. A roster speaker already confirmed on one index can be confirmed on a second only on a strong match, below 0.40. (Both rules since 15 September 2026.) On confirmation, earlier utterances of the same index within the same capture run inherit the speaker; utterances of an unconfirmed index remain unattributed."],
        ...
        { title: "2.6.5  Post-event independent check", intro: "After the event, the saved capture audio is diarized again by an independent method, which partitions the recording into speaker clusters without reference to the live indices. Each cluster is voice-identified once against the roster prints and every live utterance inside that cluster inherits the result. Clusters are classed as named (a clear match to exactly one roster print), weak, no match (this voice matches nobody enrolled), or no evidence (no turn long enough to embed). A live utterance labelled as a candidate whose cluster matches nobody enrolled is counted as wrong, not as abstained. Any non-zero count is inspected row by row and corrected. Where a corrected turn had already produced a claim, the claim moves with it -- to the corrected candidate, or to the moderator if the turn was a moderator's, so it no longer appears under any candidate's name -- and its verdict is reset and checked again (Section 7.5). A claim from a turn left with no confirmed speaker is reviewed by hand." },
        { title: "2.6.6  Measured performance", intro: "Every figure is from a named recording, so the results can be reproduced. None of these recordings was used to build a print.", items: [
          ...
          ["Arizona Superintendent of Public Instruction debate, 2026-09-15", "four capture runs after restarts. The independent check scored 591 live utterances: 557 correct and 34 wrong -- all of them moderator speech credited to a candidate in the runs after a restart, none between candidates. The cause was an index scored on its first five samples only; the re-scoring rule in 2.6.2 was added the same night. The rows were corrected before publication and no claim had been extracted from them."],
        ...
        { title: "2.6.8  What this section does not claim", items: [
          ...
          ["A false-attribution rate of zero in production", "it claims zero on the fixtures in 2.6.6 and two measured incidents, on 2026-09-09 and 2026-09-15, whose causes are fixed and whose rows were corrected before publication."],
    ...
    },
    {
      ...
      body: ["Each extracted claim is assigned a priority score from 0 to 100. The threshold governs the background ingestion queue: of the roughly 3,400 articles ingested daily, only claims scoring 30 or above enter that queue -- claims below 30 are stored but not checked by that path.", "The background queue is a sample, not the whole corpus. Extraction runs on a fixed budget of 50 articles per hourly run, chosen from eligible articles by a stated rule: articles within two days of leaving the 30-day window are taken first (oldest first), then the longest articles. An outlet's score is therefore computed on the articles that reached extraction under this rule, not on everything it published. The per-outlet verdict count and 'scored through' date make the sample size and recency visible.", "Two other paths check claims, and verdicts on an outlet's own claims from either path count toward its score exactly like verdicts from the queue. Reports are prepared in advance for a fixed list of 19 major news outlets: each ingestion run takes up to 50 of the newest articles from the past day that have unchecked claims, keeps those from listed outlets, and checks every claim on them regardless of priority score. Claims checked for a reader-requested report (below) count as well. As a result, listed outlets accumulate verdicts faster, their scores include claims below 30, and any outlet's score can include claims below 30 from requested reports.", "On-demand reports work differently. When a user submits a specific article URL, every extracted claim is checked regardless of priority score: paid reports check all extracted claims, free reports check the first 2 (ordered by claim type -- outlet claims before attributed -- not by priority score). This is a deliberate exception, not an oversight."],
      sub: [
    ...
    },
    {
      ...
      sub: [
        { title: "v1.7.1 \u2014 September 2026", intro: "Speaker Attribution (Debates) added as Section 2.6 after the first production debate; verdict-change rules stated; page corrected where it had drifted from the engine. Scoring rules, verdict types and outlet methodology are unchanged; the version stamp on claims remains v1.7.", items: [
          ["17 September 2026 -- what counts toward a score, stated in full", "Section 04 now states that claims checked in advance for a fixed list of outlets, and claims checked for reader-requested reports, count toward outlet scores whatever their priority score. This was already the engine's behaviour; the earlier text described only the background queue. No scoring rule changed."],
          ["17 September 2026 -- tiers, debate publication timing and three statements corrected", "The web leaderboard, outlet pages, report pages and browser extension counted opinion and not-verifiable verdicts toward an outlet's tier and inclusion, contrary to Sections 06 and 08; they now count scoreable verdicts only, as the mobile app and API already did. One outlet moved from Published to Stabilizing; no score changed. Since 15 September 2026 debate claims are published while the debate is in progress and the independent check in Section 2.6.5 runs after the event; Section 2.6 had described that check as running before publication. The speaker-agreement check runs on the status page and before and after every debate, not nightly as stated. Section 09 now states that three parts of each report are written by the model. Section 2.6.5 now states how a correction reaches a claim that already exists. Section 2.6 now reports the 15 September debate's measured result and the confirmation rules added after it."],
