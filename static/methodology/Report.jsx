// Report.jsx — the shared interactive methodology report component.
// Rendered inside each theme wrapper; theming is driven entirely by
// CSS custom properties on the outer `.vs-theme` container.
...
const VERDICT_META = {
  supported:      { label: "SUPPORTED",      weight: "+1.0",  tone: "pos",     blurb: "Confirmed by two independent sources (see independence rule)." },
  ...
  not_verifiable: { label: "NOT_VERIFIABLE", weight: "excl.", tone: "neutral", blurb: "Cannot confirm or deny." },
  opinion:        { label: "OPCION",        weight: "excl.", tone: "neutral", blurb: "Editorial or opinion content." },
};

function TOC({ sections, activeId, onJump, compact }) {
  return (
    <nav className="vs-toc" aria-label="Table of contents">
      ...
      <ol>
        {sections.map((s) => (
          <li key={s.id}>
            <a href={`#${s.id}`} className={cx("vs-toc__link", activeId === s.id && "is-active")}
              onClick={() => onJump(s.id)}>
      ...
      {!compact && (
        <div className="vs-toc__footer">
          <div className="vs-toc__brand">VERUM SI@�AF</div>
      ...
      <div className="vs-section__body">
        ...
        {s.kind === "verdicts" && s.verdicts && (
          <div className="vs-legend">
            ...
            {s.verdicts.map(v => (
              <div key={v.key} className="vs-legend__row">
        ...
        {s.callout && (
          <aside className="vs-callout">
            <div className="vs-callout__label">{callout.label || 's.callout.label'}</div>
