...
def upgrade_submit():
    ...
    email_body = '<html><body style="font-family:sans-serif;color:#333;max-width:520px;margin:0 auto;padding:24px;">' \
        ...
        '<p style="color:#999;font-size:11px;border-top:1px solid #eee;padding-top:12px;margin-top:24px;">Verum Signal &middot; Methodology v1.7.1</p>' \
        '</body></html>'
...
<body>
  <div class="wrap">
    ...
    <div class="tagline">METHODOLOGY v1.7.1 &nbsp;·&nbsp; AUTOMATED &nbsp;·&nbsp; AUDITABLE</div>
  </div>
  <script>
    function analyse() {
...
setTimeout(checkStatus, 3000);
            ...
            # requests may trigger a NEW fetch. Sec-Fetch-Site is browser-
            # set, not forgeable by JS — the loading page's own polling
            # fetch() is same-origin regardless of how the user arrived,
            # so genuine users see zero added friction. Already-cached
            # articles never reach this branch at all (see module
  ...
  <div class="stage">
    ...
    <div class="panel">
      ...
      <p class="reset-note">Free reports reset daily &middot; No card required for Pro trial</p>
            ...
            source = source_name  # ensure 'source' is defined before any lambda captures it (paths B/C)
            ...
            if _should_reverify:
                ...
                try:
                    ...
                    if not _fetch_failed and not claims:
                        ...
                        from auth_routes import reserve_quota, release_quota
                        _quota_b_reserved = False
                        if _gate_user:
                            # Atomic reserve — same fix as path A, same reason:
                            # check_quota()-then-increment_quota() was confirmed
                            # racy (session6_quota_race_test.py). release_quota()
                            # below refunds if verify_and_insert_claims() fails.
                            _quota_b = reserve_quota(get_db, _gate_user["id"], "consumer")
...
<div class="wrap">
  ...
  <div class="card">
    ...
    def claim_row(c, idx):
        ...
        if src_structured:
            ...
            _NAME_MAP = {
                ...
                'al jazeera': ('Al Jazeera', 'aljazeera.com'), 'aljazeera': ('Al Jazeera', 'aljazeera.com'),
                'pbs newshour': ('PBS NewsHour', 'pbs.org'), 'pbs': ('PBS', 'pbs.org'),
                'britannica': ('Britannica', 'britannica.com'),
                'cfr': ('CFR', 'cfr.org'), 'council on foreign relations': ('CFR', 'cfr.org'),
                'ynet': ('Ynet News', 'ynetnews.com'), 'ynet news': ('Ynet News', 'ynetnews.com'),
                'newsweek': ('Newsweek', 'newsweek.com'),
                'france 24': ('France 24', 'france24.com'),
                'msnbc': ('MSNBC', 'msnbc.com'),
                'the atlantic': ('The Atlantic', 'theatlantic.com'),
                'time': ('Time', 'time.com'),
                'usa today': ('USA Today', 'usatoday.com'),
                'csis': ('CSIS', 'csis.org'),
                'brookings': ('Brookings', 'brookings.edu'),
                'foreign policy': ('Foreign Policy', 'foreignpolicy.com'),
                'defense one': ('Defense One', 'defenseone.com'),
                'military times': ('Military Times', 'militarytimes.com'),
                'fast company': ('Fast Company', 'fastcompany.com'),
                'gulf news': ('Gulf News', 'gulfnews.com'),
                'house of commons': ('House of Commons Library', 'commonslibrary.parliament.uk'),
                'vox': ('Vox', 'vox.com'),
                'propublica': ('ProPublica', 'propublica.org'),
                'the intercept': ('The Intercept', 'theintercept.com'),
                'huffpost': ('HuffPost', 'huffpost.com'),
                'daily beast': ('Daily Beast', 'thedailybeast.com'),
                'business insider': ('Business Insider', 'businessinsider.com'),
                'nbc': ('NBC News', 'nbcnews.com'),
            }
            src_labels = []
            sources_lower = sources.lower()
            _matched = set()
        ...
        try:
            ...
            if len(bg_facts) >= 3:
                ...
                background_signal_html = (
                    ...
                    'letter-spacing:0.16em;color:#a855f7;'
    ...
    try:
        ...
        _prompt = ('You are the editorial analysis layer for Verum Signal, an independent claim analysis platform governed by Methodology v1.7.1.' + chr(10) +
    ...
    if _verdict_count >= 20 and outlet_score is not None:
        ...
        outlet_history_line = (
            ...
            '(' + str(_verdict_count) + '/20 verdicts collected, see <a href=\"/methodology\" target=\"_blank\">Methodology v1.7.1</a>)'
        )
    ...
    _sc_total_n = stats.get('total', 0)
    html = html.replace('{{sc_total}}', str(_sc_total_n))
    html = html.replace('{{sc_total_label}}', 'claim' if _sc_total_n == 1 else 'claims')
...
<div class="deploy-entry">
  ...
  <ul class="deploy-items">
    ...
    <li><strong>Non-news exclusion:</strong> 27 non-news domains excluded from the leaderboard and from the public source endpoints (exact hostname match &mdash; gao.gov, justice.gov, PR wires, non-news entities). EXCLUDED_DOMAINS frozenset in api_leaderboard.py.</li>
...
<div class="section">
  ...
  <div class="cards">
    ...
    <div class="card">
      <div class="card-num"><span class="pill">v1.7.1</span></div>
