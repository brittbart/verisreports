...
def process_articles_from_db(limit=50, min_content_chars=500, days_window=30):
    ...
    Reads unextracted articles directly from the DB rather than from
    today's fetch JSON file. Eliminates the fetch-overwrite-drop pattern
    where most ingested articles never got extracted.

    Parameters
    ----------
    limit : int
        Max articles to process this run. The existing per-run cap.
    min_content_chars : int
        Skip articles whose content is shorter than this. Quality filter,
        genre-agnostic.
    days_window : int
        Only consider articles fetched within this many days. Avoids
        attempting to extract from very old, possibly-stale content.

    Output
    ------
    Writes claims_YYYY-MM-DD.json in the same format process_articles()
    uses, so the existing load_to_database.load_articles() pipeline
    consumes it unchanged.

    Returns
    -------
    list of result dicts, one per article that yielded at least one claim.
    """
    import os
    import psycopg2
    from psycopg2.extras import RealDictCursor

    # Same connect pattern stages/_common.py uses.
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(sql, (min_content_chars, str(days_window), limit))
    ...
    for i, row in enumerate(rows):
        # Build the dict shape extract_claims_from_article() expects.
        article_dict = {
            "title": row["title"],
            "url": row["url"],
